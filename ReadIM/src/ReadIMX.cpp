...
int GetVectorComponents( BufferFormat_t imageSubType )
{
	...
	if (imageSubType >= BUFFER_FORMAT_VECTOR_2D_EXTENDED && imageSubType <= BUFFER_FORMAT_VECTOR_3D_EXTENDED_PEAK)
		return compN[imageSubType];
	return 0;
}
...
int WriteIMGXAttr( const char* theFileName, bool isIMX, BufferType* myBuffer, AttributeList* myList )
{
	...
	// create comment
	strcpy( header.com1, "Created by ReadIMX.dll" );
	strcpy( header.com2, "" );
